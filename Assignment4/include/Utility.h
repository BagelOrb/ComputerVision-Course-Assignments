...
class Utility
{
	...
	struct UniqueValue_d
	{
		...
		UniqueValue_d(const double current = 0, const double max = 1, const double step = 1) :
				_current(current), _max(max), _step(step)
		{
			_size = (int) floor((_max - _current) / _step);
		}
...
public:
	...
	static void meshgrid(const cv::Range &rx, const cv::Range &ry, T &X, T &Y)
	{
		...
		case CV_64F:
		{
			...
			for (int i = rx.start; i <= rx.end; i++)
				t_x.push_back((double) i);
			for (int i = ry.start; i <= ry.end; i++)
				t_y.push_back((double) i);
			meshgrid(t_x, t_y, X, Y);
		}
			break;
		default:
	...
	{
		cv::repeat(cv::Mat(rx).reshape(1, 1), (int) ry.size(), 1, X);
		cv::repeat(cv::Mat(ry).reshape(1, 1).t(), 1, (int) rx.size(), Y);
	}
	...
	static void getImagesCanvas(const int &, const cv::Mat &, const cv::Size &, cv::Mat &, const cv::Scalar = Color_RED);
	static void getImagesCanvas(const cv::Size &, const cv::Mat &, const cv::Size &, cv::Mat &, const cv::Scalar &);
	static void getHeatmap(const cv::Mat&, cv::Mat &, int = 10, int = 0, int = cv::COLORMAP_JET, bool = true);
};
