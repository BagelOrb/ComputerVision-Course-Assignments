		...
		const std::string prefix, const std::string postfix, const bool is_dir_only)
{
	...
	auto app_path = p_path.append("\\*");
	hFind = FindFirstFile(std::wstring(app_path.begin(), app_path.end()).c_str(), &fdata);

	if (hFind != INVALID_HANDLE_VALUE)
	{
		do
		{
			auto wfilename = std::wstring(fdata.cFileName);
			auto sfilename = std::string(wfilename.begin(), wfilename.end());

			if (!(sfilename == "." || sfilename == ".."))
			{
				if (is_dir_only && (fdata.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
				{
					if (!mask.empty() && boost::regex_match(sfilename, boost::regex(mask)))
					files.push_back(prefix + sfilename + postfix);
				}
				else if (!is_dir_only)
				{
					if (!mask.empty() && boost::regex_match(sfilename, boost::regex(mask)))
