...
cv::Mat Utility::box_overlap(const cv::Mat &a, const cv::Rect &b)
{
	...
	for (int r = 0; r < a.rows; ++r)
	{
		int width = int(a.at<double>(r, 2) - a.at<double>(r, 0) + 1);
		int height = int(a.at<double>(r, 3) - a.at<double>(r, 1) + 1);
	...
	for (size_t l = 0; l < labels_w.size(); ++l)
		o.at<double>(labels_w[l]) = 0;
	mat_labels_lteq(h, 0, labels_h);
	for (size_t h = 0; h < labels_h.size(); ++h)
...
#ifndef _WIN32
	tm tm_buf;
	struct tm* pt = localtime_r(&in_time, &tm_buf);
	struct tm t = *pt;
#else
	struct tm t;
...
std::string Utility::get_time_progress(const int index, const int total, std::vector<double> &t_list)
{
	...
	if (mfps > 0)
	{
		...
		if (time_left < std::numeric_limits<double>::max()) eta = get_timestamp(time(NULL) + (time_t) cvRound(time_left));
...
void Utility::meshgrid(const std::vector<std::vector<double>> &r, std::vector<cv::Mat*> &M)
{
	...
	for (size_t i = 0; i < r.size(); ++i)
		m_size.push_back((int) r[i].size());

	for (size_t i = 0; i < r.size(); ++i)
	{
		cv::Mat m((int) r.size(), &m_size[0], CV_64F, cv::Scalar::all(0));
		...
		for (size_t j = 0; j < idx[i].size(); ++j)
		{
			...
			for (size_t r = 0; r < idx.size(); ++r)
			{
				if (r == i)
				{
					...
					range1[i] = cv::Range((int) j, (int) j + 1);
				}
				else
				{
...
void Utility::whiten(const cv::Mat &src, const cv::Size &s_size, cv::Mat &dst)
{
	...
	cv::Mat filt_shifted_1D(1, (int) filt_shifted.total(), filt_shifted.type());
...
void Utility::getImagesCanvas(const int &c_size, const cv::Mat &data, const cv::Size &size, cv::Mat &canvas, const cv::Scalar border_color)
{
	getImagesCanvas(cv::Size(c_size, c_size), data, size, canvas, border_color);
}

/**
 * Creates a canvas of multiple images (collage)
 */
void Utility::getImagesCanvas(const cv::Size &c_size, const cv::Mat &data, const cv::Size &size, cv::Mat &canvas, const cv::Scalar &border_color)
{
	...
	for (int i = 0; i < canvas_total; ++i)
	{
		...
		if (tmp_im.channels() == 1)
		{
			...
			cvtColor(tmp_sz_im, tmp_sz_im, CV_GRAY2BGR);
		}
		else if (tmp_im.channels() == 3) 
		{
			tmp_im.convertTo(tmp_sz_im, CV_8UC3);
		}
		...
		if (i % canvas_cols == 0)
		{
			...
			{
				canvas = canvas_line;
				cv::Mat l(1, canvas.cols, CV_8UC3, border_color);
				if (canvas.empty())
					canvas = l;
				else
					vconcat(canvas, l, canvas);
			}
			else
			{
				vconcat(canvas, canvas_line, canvas);
				canvas_line.release();
				cv::Mat l(1, canvas.cols, CV_8UC3, border_color);
				vconcat(canvas, l, canvas);
			}

			tmp_sz_im.convertTo(canvas_line, CV_8UC3);
		}
		else
		{
			hconcat(canvas_line, tmp_sz_im, canvas_line);
		}

		cv::Mat l(canvas_line.rows, 1, CV_8UC3, border_color);
		hconcat(canvas_line, l, canvas_line);
	}

	if (!canvas_line.empty())
	{
		const int width = canvas.cols - canvas_line.cols;
		if (width > 0)
		{
			cv::Mat f = cv::Mat::zeros(canvas_line.rows, width, CV_8UC3);
			hconcat(canvas_line, f, canvas_line);
		}
		vconcat(canvas, canvas_line, canvas);
	}
}
 ...
 */
void Utility::getHeatmap(const cv::Mat &src, cv::Mat &dst, int size, int offset, int color_map, bool normalize)
{
	...
	{
		int height = (int) cvRound(src.rows * (target_width / (double) src.cols));
	...
	for (int y = 0; y < proc_img.rows; ++y)
	{
		for (int x = 0; x < proc_img.cols; ++x)
		{
			cv::rectangle(heatmap, cv::Point(x * size, y * size), cv::Point(x * size + size, y * size + size),
					cv::Scalar::all(cvRound(proc_img.at<float>(y, x))), CV_FILLED);
		}
	}
